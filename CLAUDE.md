...
- **第5ラウンド（最終調整）**: ブロック化後も実機で「末尾で止まろうとして止まりきれない」感触が残ったため、`.app-page` の縦スナップを **`y mandatory` → `y proximity`** に変更（v2026-07-04g）。吸着点の近くで離したときだけスナップし、それ以外は完全に自由。Chromium検証では入り口吸着・最下部静止・回帰22項目すべて維持

...
最終更新: 2026-07-07（ヒーロー画像を圧縮・未参照ファイルを削除。詳細は「0-6. ヒーロー画像圧縮＋未参照ファイル削除」参照）
...
11. **管理画面スマホ表示の実機最終確認**（2026-07-05実装分。ヘッダー「管理者用画面」表示・ナビ非表示・フッター「直前の画面に戻す」リンクが実機スマホ幅で崩れないか）
12. **カルーセル1枚目の画像を別の画像に差し替え**（2026-07-05追加。上記5番とは別の、さらなる差し替え依頼。画像ファイルの準備が必要）

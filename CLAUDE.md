...
**追記（同日）— ゲスト利用数が「延べ数」である旨の注記を追加:** 「ゲスト利用数」は匿名認証アカウントの累計作成数であり、同一人物が端末を変える・ログオフ→再度ゲスト開始する、のたびに新しい匿名アカウントが作られてカウントが増える「延べ数」（ユニーク利用者数ではない）という性質を持つ。誤解防止のため`.stat-card`内に`.note`（10px・`var(--muted)`）を新設し、ゲスト利用数カードに「※延べ数（同一人物が複数回ゲスト利用した場合も加算されます）」を追加。本番デプロイ後、カード内で2行に折り返して表示され他カードとのレイアウト崩れが無いことを確認済み。

---

## 0-9. トップページ文言更新・購入済みユーザー向け再ログイン導線（2026-07-08）

いずれも `index.html` のみの変更。3コミットに分けて本番デプロイ済み（https://apps100kin.web.app ）。

**① PWA説明文差し替え＋利用上の注意文追加（コミット 23cc6cc）:**
- 緑枠（`.sync-note`、「開発の動機」セクション末尾）の文言を「PCからも、スマホからも、同じURL〜」→「このアプリはインストールの手間がない身軽で、ブラウザと同じ安全性の仕組みで守られているPWAアプリです」に差し替え
- 緑枠直下に注意文 `.privacy-caution`（「リスクは0ではないです。メモの中には個人情報は入れないようにしてください。情報の紛失・漏洩について、開発者は責任を負いかねます。」）を新規追加
- ⚠️ **「開発の動機」①〜⑥の本文はindex.html内に存在しない**。Firestore `apps/memo-sync` ドキュメントの `features` フィールド（配列）から動的描画されるため、差し替えはFirebase Consoleでの編集が必要（2026-07-08時点で新文言への差し替えは**ユーザー対応待ち・未実施**。編集後はページ再読み込みのみで反映、デプロイ不要）

**② 緑枠・注意文の書式調整（コミット e963d61）:**
- `.sync-note`: 中央揃え→左詰め（太字は既存の `font-weight:700` を維持）
- `.privacy-caution`: 中央揃え→左詰め、色 `#8b8b80`→`#4d4d45`（「ほとんど見えない」フィードバック対応）、サイズ `clamp(11px,2.9vw,13px)`→`clamp(12px,3.1vw,14px)`

**③ 購入済みなのに使えない方向けの再ログイン案内リンク（コミット 88963ab）:**
- 詳細モーダルの「ゲストで試してみる」ボタン下・「アプリのレビューを投稿する↓」上に `.purchased-help-link`（「購入済みなのに使えない方は、こちらから再度お試しください」）を追加
- リンク先: `https://crossmemo.web.app/?payment=success`（`target="_blank"`）。決済成功モーダル→Googleログイン→isPremium付与のやり直し導線
- ⚠️ **既知のトレードオフ（開発者了承済み）**: このURLは決済の有無を検証しないため、未購入者もこのリンク→Googleログインで無料でisPremiumを取得できる。根本対策はStripe Webhook + Cloud Functionsによる決済検証（howto-v2側CLAUDE.mdの「次のステップ」残タスク）
